...
Then read `/agent-context` again for the next loop.
...
- reflection, human notes, rollback markers, and lineage.
...
9. Finalize the loop only after memory completeness checks pass.

Rollback must use the repository/API rollback path so later loops remain visible
as abandoned branches.
